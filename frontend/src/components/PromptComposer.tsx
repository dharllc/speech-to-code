...
const PromptComposer: React.FC<PromptComposerProps> = ({ 
  ...
  const [isAutoAnalyzeEnabled, setIsAutoAnalyzeEnabled] = useState<boolean>(() => 
  ...
  const [enhancementDisabled, setEnhancementDisabled] = useState<boolean>(() => 
    JSON.parse(localStorage.getItem('enhancementDisabled') || 'false')
  );
  const [isAutoCopyEnabled, setIsAutoCopyEnabled] = useState<boolean>(() => 
      ...
      if (file.type === 'directory' && file.files) {
        for (const subFile of file.files) {
          if (!newContents[subFile.path]) {
            try {
              const response = await axios.get<FileContentResponse>(`${API_URL}/file_content?repository=${selectedRepository}&path=${subFile.path}`);
        ...
        if (!newContents[file.path]) {
          try {
            const response = await axios.get<FileContentResponse>(`${API_URL}/file_content?repository=${selectedRepository}&path=${file.path}`);
            if (response.data.is_binary) {
