...
type StageStatus = 'complete' | 'in-progress' | 'locked' | 'not-started';
type ScoreType = 'clarity' | 'feasibility' | 'instruction';

const StageProgress: React.FC<StageProgressProps> = ({ stageHistory }) => {
  if (!stageHistory || stageHistory.length === 0) return null;

  const getLatestStageScore = (stage: StageHistoryEntry['stage']): StageHistoryEntry | undefined => {
  ...
  const getScoreColor = (score: number, type: ScoreType = 'clarity'): string => {
    if (type === 'clarity') {
  ...
  const getStageStatus = (stage: StageHistoryEntry['stage']): StageStatus => {
    if (stage === 'stage1-understand-validate') {
      if (!stage1Latest) return 'not-started';
      return (stage1Latest.clarityScore ?? 0) >= 90 ? 'complete' : 'in-progress';
    } else if (stage === 'stage2-plan-validate') {
      if (!stage1Latest || (stage1Latest.clarityScore ?? 0) < 90) return 'locked';
      if (!stage2Latest) return 'not-started';
      return (stage2Latest.feasibilityScore ?? 0) >= 90 ? 'complete' : 'in-progress';
    } else if (stage === 'stage3-agent-instructions') {
      if (!stage2Latest || (stage2Latest.feasibilityScore ?? 0) < 90) return 'locked';
      if (!stage3Latest) return 'not-started';
      return (stage3Latest.instructionQuality ?? 0) >= 90 ? 'complete' : 'in-progress';
    }
    return 'not-started';
  };
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-grow">
          <div className="flex items-center justify-between">
            ...
            {stage1Latest && stage1Latest.clarityScore !== undefined && (
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-grow">
          <div className="flex items-center justify-between">
            ...
            {stage2Latest && stage2Latest.feasibilityScore !== undefined && (
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-grow">
          <div className="flex items-center justify-between">
            ...
            {stage3Latest && stage3Latest.instructionQuality !== undefined && (
        ...
        <div className="space-y-2">
          {stageHistory.map((entry, index) => (
            <div key={index} className="flex items-start gap-2 text-xs">
              ...
              <div>
                {entry.stage === 'stage1-understand-validate' && entry.clarityScore !== undefined && (
                  <span className={getScoreColor(entry.clarityScore, 'clarity')}>
                    Clarity Score: {entry.clarityScore}
                  </span>
                )}
                {entry.stage === 'stage2-plan-validate' && entry.feasibilityScore !== undefined && (
                  <span className={getScoreColor(entry.feasibilityScore, 'feasibility')}>
                    Feasibility Score: {entry.feasibilityScore}
                  </span>
                )}
                {entry.stage === 'stage3-agent-instructions' && entry.instructionQuality !== undefined && (
