 ...
 */
const getAllFilesInFolder = (node, fullPath, repositoryName) => {
  let collected = [];
 ...
 */
const getFolderSelectionState = (node, fullPath, selectedFiles, repositoryName) => {
...
const RepositoryFileViewer = ({ selectedRepository, onFileSelect, selectedFiles }) => {
  ...
  const renderTree = (node, path = '', depth = 0) => {
    ...
    const getFolderControlIcon = (state) => {
      switch (state) {
        case 'all':
