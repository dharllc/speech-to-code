...
const SystemPromptManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingPrompt) await axios.put(`${API_URL}/system_prompts/${editingPrompt.id}`, promptData);
      else await axios.post(`${API_URL}/system_prompts`, promptData);
      fetchPrompts();
  ...
  const handleEdit = (prompt) => {
    ...
    setNewPrompt({ name: prompt.name, step: prompt.step, content: prompt.content, is_default: prompt.is_default });
        ...
        <div ref={formRef} className={`bg-white dark:bg-gray-800 rounded-xl shadow-lg transition-all duration-300 ${isFormCollapsed ? 'h-16 overflow-hidden' : ''}`}>
          ...
          <form onSubmit={handleSubmit} className="p-4 pt-0">
            <div className="flex gap-4 mb-4">
              <input type="text" name="name" value={newPrompt.name} onChange={handleInputChange} placeholder="Prompt Name" className="flex-1 px-4 py-2 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 outline-none transition-all" required />
              <input type="text" name="step" value={newPrompt.step} onChange={handleInputChange} placeholder="Step (e.g., 1)" className="w-32 px-4 py-2 rounded-lg bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 outline-none transition-all" required />
            </div>
          ...
          {prompts.map((prompt) => (
            <div key={prompt.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 transition-transform hover:scale-[1.01]">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-gray-900 dark:text-white flex items-center gap-2">{prompt.name}<span className="text-sm px-2 py-1 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300">{prompt.step}</span></h3>
              ...
              <pre className="text-sm mb-4 bg-gray-50 dark:bg-gray-900 p-4 rounded-lg overflow-auto whitespace-pre-wrap break-words min-h-[50px] max-h-[500px]">{prompt.content}</pre>
              <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
                <span className="flex items-center gap-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg>{prompt.token_count} tokens</span>
                <span className="flex items-center gap-1"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{new Date(prompt.timestamp).toLocaleString()}</span>
              </div>
            </div>
          ))}
        </div>
