...
const PromptComposer: React.FC<PromptComposerProps> = ({ 
  ...
  const [isTreeAdded, setIsTreeAdded] = useState<boolean>(false);
  const [treeTokenCount, setTreeTokenCount] = useState<number>(0);
  const [hasUnsavedChanges, setHasUnsavedChanges] = useState<boolean>(false);
  const [fileSuggestions, setFileSuggestions] = useState<FileSuggestionResponse['suggestions'] | null>(null);
  const [isAnalyzing, setIsAnalyzing] = useState<boolean>(false);
  const [isAutoAnalyzeEnabled, setIsAutoAnalyzeEnabled] = useState<boolean>(false);
  const [addedBatches, setAddedBatches] = useState<string[]>([]);
  const [autoAddEnabled, setAutoAddEnabled] = useState<boolean>(false);
  const [enhancementDisabled, setEnhancementDisabled] = useState<boolean>(false);
  const [isAutoCopyEnabled, setIsAutoCopyEnabled] = useState<boolean>(false);

  const MIN_PROMPT_LENGTH = 20;
  const lastPromptRef = useRef<string>(basePrompt);
  const treeOperationRef = useRef<boolean>(false);
  const lastCopiedPromptRef = useRef<string>('');
  ...
  useEffect(() => {
    ...
    setIsAutoAnalyzeEnabled(JSON.parse(localStorage.getItem('autoAnalyzeEnabled') || 'false'));
    setAutoAddEnabled(JSON.parse(localStorage.getItem('autoAddEnabled') || 'false'));
    setEnhancementDisabled(JSON.parse(localStorage.getItem('enhancementDisabled') || 'false'));
    setIsAutoCopyEnabled(JSON.parse(localStorage.getItem('autoCopyEnabled') || 'false'));
  }, []);
    ...
    Object.keys(newContents).forEach(path => {
      const stillSelected = selectedFiles.some(file => {
        if (file.type === 'directory') {
          return file.files?.some(f => f.path === path) || false;
        }
      ...
      if (file.type === 'directory' && file.files) {
        for (const subFile of file.files) {
          if (!newContents[subFile.path]) {
            try {
              const response = await axios.get<FileContentResponse>(`${API_URL}/file_content?repository=${selectedRepository}&file_path=${subFile.path}`);
        ...
        if (!newContents[file.path]) {
          try {
            const response = await axios.get<FileContentResponse>(`${API_URL}/file_content?repository=${selectedRepository}&file_path=${file.path}`);
            if (response.data.is_binary) {
  ...
  const removeFile = (filePath: string) => {
    ...
    if (fileToRemove) {
      if (fileToRemove.type === 'directory' && fileToRemove.files) {
  ...
  //  TRANSCRIPTION
  // ======================
  const addToPrompt = useCallback((text: string) => {
    if (!text) return;
  ...
  //   BATCH ADD/REMOVE
  // ======================
  const handleBatchAdd = (batchKey: keyof NonNullable<typeof fileSuggestions>) => {
    if (!fileSuggestions || !fileSuggestions[batchKey]) return;
    fileSuggestions[batchKey].forEach((item: FileSuggestion) => {
  ...
  const handleBatchRemove = (batchKey: keyof NonNullable<typeof fileSuggestions>) => {
    if (!fileSuggestions || !fileSuggestions[batchKey]) return;
    fileSuggestions[batchKey].forEach((item: FileSuggestion) => {
      if (selectedFiles.some(file => file.path === item.file)) {
  ...
  const getFileChips = () => {
    ...
    const sortedFiles = [...selectedFiles].sort((a, b) => {
      const aTokens = a.type === 'directory'
        ? (typeof a.token_count === 'object' ? a.token_count.total : (a.token_count || 0))
        : (fileContents[a.path]?.tokenCount || 0);
      const bTokens = b.type === 'directory'
        ? (typeof b.token_count === 'object' ? b.token_count.total : (b.token_count || 0))
      ...
      if (file.type === 'directory') {
        return (
          <FileChip
            key={file.path}
            fileName={file.path}
            tokenCount={typeof file.token_count === 'object' ? file.token_count.total : (file.token_count || 0)}
            onRemove={() => removeFile(file.path)}
            isBinary={false}
            isRepositoryTree={false}
          />
        );
      } else {
  ...
  const handleRestoreCombination = useCallback(async (combination: FileCombination) => {
    ...
    try {
      ...
      const newContents: Record<string, FileContent> = {};
      const filesToAdd: SelectedFile[] = [];
      
      for (const file of combination.files) {
        if (file.type === 'directory' && file.files) {
          const directoryFiles: SelectedFile['files'] = [];
          for (const subFile of file.files) {
            try {
              const response = await axios.get<FileContentResponse>(`${API_URL}/file_content?repository=${selectedRepository}&file_path=${subFile.path}`);
              if (!response.data.is_binary) {
      ...
      <Card className="shadow border-0">
        <CardContent className="p-6">
          ...
          <PromptActions
            ...
            setTranscription={(text: string) => text && enhanceTranscription(text)}
            setStatus={setStatus}
            ...
            <PromptTextArea
              ...
              additionalTokenCount={
                ...
                selectedFiles.reduce((sum, file) => {
                  if (file.type === 'directory') {
                    const tokenCount = typeof file.token_count === 'object' 
                      ? file.token_count.total 
                      : (file.token_count || 0);
