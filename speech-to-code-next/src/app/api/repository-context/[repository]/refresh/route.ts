...
export async function POST(
  ...
  try {
    ...
    // Generate and save context map (refresh = regenerate)
    const contextMap = generateContextMap(repoPath, repository);
    saveContextMap(contextMap, CONTEXT_MAPS_DIR);

    return NextResponse.json({
      message: 'Context map refreshed successfully',
      repositoryId: repository
    });
  } catch (error) {
    console.error('Error refreshing context map:', error);
    return NextResponse.json(
      { error: `Failed to refresh context map: ${error instanceof Error ? error.message : 'Unknown error'}` },
      { status: 500 }
    );
  }
}